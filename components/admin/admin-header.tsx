  ...
  return (
    <header className="sticky top-0 z-30 flex h-20 w-full items-center justify-between border-b border-slate-200/80 bg-white/90 dark:bg-card/90 px-3 sm:px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2 sm:gap-4 flex-1 max-w-md">
        <button
          onClick={handleMenu}
          className="text-slate-500 hover:text-slate-800 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-muted transition-colors cursor-pointer shrink-0"
          aria-label="Toggle navigation"
        >
          <Menu className="size-5" />
        </button>

        <div className="relative w-full max-w-[160px] xs:max-w-[220px] sm:max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-3.5 sm:size-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full rounded-xl border border-slate-200 bg-slate-50/80 dark:border-border dark:bg-muted/40 pl-8 sm:pl-10 pr-3 sm:pr-12 py-1.5 sm:py-2 text-xs font-medium outline-none focus:border-[#2E6F40] focus:bg-white dark:focus:bg-card transition-colors"
          />
          <kbd className="hidden sm:inline-block absolute right-3 top-1/2 -translate-y-1/2 rounded-md border border-slate-200 bg-white dark:border-border dark:bg-card px-1.5 py-0.5 text-[10px] font-semibold text-slate-400 shadow-xs">
            ⌘K
          </kbd>
        </div>
      </div>
