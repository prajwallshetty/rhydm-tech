...
import { Lock, Mail, User, ShieldCheck, Loader2, AlertCircle, CheckCircle2, XCircle } from "lucide-react";
import { signupAction } from "@/app/(backend)/(auth)/actions";
      ...
      >
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs font-semibold text-muted-foreground">
            Join Rhydm Tech to access IT disposal quotes and refurbished store warranties.
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-8 shadow-xl backdrop-blur-md dark:border-border dark:bg-card space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-3.5 dark:border-border dark:bg-muted/30 space-y-1.5 text-[11px] font-bold text-slate-600 dark:text-muted-foreground">
              <div className="text-[10px] uppercase tracking-wider text-slate-400 font-extrabold mb-1">
                Password Security Checklist:
              </div>
              <div className="grid grid-cols-2 gap-1.5">
                <div className="flex items-center gap-1.5">
                  {hasLength ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={hasLength ? "text-slate-900 dark:text-white" : ""}>Min 8 characters</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {hasUpper ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={hasUpper ? "text-slate-900 dark:text-white" : ""}>1 Uppercase letter</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {hasLower ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={hasLower ? "text-slate-900 dark:text-white" : ""}>1 Lowercase letter</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {hasNumber ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={hasNumber ? "text-slate-900 dark:text-white" : ""}>1 Number</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {hasSpecial ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={hasSpecial ? "text-slate-900 dark:text-white" : ""}>1 Special char (@$!%*?&)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {isMatch ? <CheckCircle2 className="size-3.5 text-emerald-500" /> : <XCircle className="size-3.5 text-slate-300" />}
                  <span className={isMatch ? "text-slate-900 dark:text-white" : ""}>Passwords match</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              />
              <label htmlFor="acceptTerms" className="text-xs font-bold text-slate-600 dark:text-muted-foreground select-none cursor-pointer">
                I accept the{" "}
                <Link href="/terms" className="text-[#2E6F40] underline">
                  Terms of Service
                </Link>{" "}
                and{" "}
                <Link href="/privacy" className="text-[#2E6F40] underline">
                  Privacy Policy
                </Link>
              </label>
            </div>
          ...
          <div className="text-center pt-2 text-xs font-semibold text-muted-foreground">
            ...
            <Link href="/login" className="font-extrabold text-[#2E6F40] hover:underline">
              Sign in
            </Link>
          </div>
        </div>
